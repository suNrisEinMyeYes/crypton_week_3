...
contract Unique is ERC1155, Ownable {

    using Strings for uint256;

    uint256 public constant ODINS_EYE = 0;
    uint256 public constant THORS_HAMMER = 1;
    uint256 public constant LOKIS_BOOK = 2;
    uint256 public constant VALKYRIAS_FEATHER = 3;
    uint256 public constant YOTUNS_AXE = 4;

    string public name;
  // Contract symbol
    string public symbol;
    uint256[] supplies = [1,1,1,1,1];
    uint256[] minted = [0,0,0,0,0];

    constructor(string memory _name, string memory _symbol) ERC1155("https://ipfs.io/ipfs/Qmf1b55qxKEm1MfKUcTni4YV1a6TgabKR1WvFJkUzRsUSg/{id}.json") {
        name = _name;
        symbol = _symbol;
    }

    function mint( address account, uint256 id, uint256 amount, bytes memory data) public onlyOwner { 
        require(supplies[id]>0);
        supplies[id] -= amount;
    ...
    function mintBatch( address account, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public onlyOwner { 
        ...
        for (uint256 index = 0; index < ids.length; index++) {
            require(supplies[index]>0);
            supplies[index] -= amounts[index];
        }
    ...
    function uri(uint256 _tokenId) override public pure returns(string memory){
        return string(abi.encodePacked("https://ipfs.io/ipfs/Qmf1b55qxKEm1MfKUcTni4YV1a6TgabKR1WvFJkUzRsUSg/", _tokenId.toString(), ".json" ));
    }
